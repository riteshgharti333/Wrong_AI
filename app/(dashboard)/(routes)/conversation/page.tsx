...
const ConversationPage = () => {
  ...
  return (
    <div>
      <Heading
        title="Conversation"
        description="Our most advances conversation model."
        icon={MessageSquare}
      ...
      <div className="px-4 lg:px-8">
                ...
                render={({ field }) => (
                  <FormItem className="col-span-12 lg:col-span-10">
                    <FormControl className="m-0 p-0">
                        ...
                        disabled={isLoading}
                        placeholder="Write js code for sum 3 values"
                        {...field}
                        />
                    </FormControl>
                  </FormItem>
        ...
        {isLoading && (
          <div className="p-8 rounded-lg w-full dlex items-center justify-center bg-muted">
            <Loader />
          </div>
        )}    
         ...
         <div className="flex flex-col-reverse gap-y-4">
          {messages.map((message) => (
            <div 
            key={message.content}
            className={cn ("p-8 w-full flex items-start gap-x-8 rounded-lg" , message.role === "user" ? "bg-white border border-blacl/10" : "bg-muted")}
            >
